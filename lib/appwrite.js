...
export const likeRecipe = async (userId, recipeId) => {
    try {
        const recipeDocument = await databases.getDocument(databaseId, recipeCollectionId, recipeId);

        // Update the liked field by adding the userId (Array needed)
        const likedArray = recipeDocument.liked || [];
        ...
        const updatedRecipe = await databases.updateDocument(
            databaseId,
            recipeCollectionId,
            recipeId, // Using recipe id as document id
            {
                liked: likedArray
            }
        );
...
export const userLikedRecipes = async (userId) => {
    try {
        const likedRecipes = await databases.listDocuments(
            databaseId,
            recipeCollectionId,
            [
